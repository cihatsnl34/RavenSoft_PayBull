  ...
  <h1>Şirket ismi : {{$data->companyName}}</h1><br><br>
    <form action="{{ route('admin.admin_documents_add',['applications_id'=>$data->id]) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-8">
        ...
        <input id="file-upload" class="block h-12 w-full py-2 px-3 appearance-none border border-coolGray-200 rounded-lg shadow-md text-coolGray-300 leading-6 placeholder-coolGray-300 focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-green-500 focus:ring-opacity-50" name = "adresBelgesi" type="file" placeholder="Dosya Seçilmedi"></div>
      <div class="flex flex-wrap -mx-4 mb-8">
        <div class="w-full md:w-1/3 px-4 mb-8 md:mb-0">
          <label class="block mb-2 text-coolGray-800 font-medium leading-6" for="">İmza Sirküleri</label>
          <input id="file-upload" name = "imzaSirküleri" class="block h-12 w-full py-2 px-3 appearance-none border border-coolGray-200 rounded-lg shadow-md text-coolGray-300 leading-6 placeholder-coolGray-300 focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-green-500 focus:ring-opacity-50" type="file" placeholder="Dosya Seçilmedi"></div>
        <div class="w-full md:w-1/3 px-4 mb-8 md:mb-0">
          <label class="block mb-2 text-coolGray-800 font-medium leading-6" for="">Vergi Levhası</label>
          <input id="file-upload" name = "vergiLevhasi" class="block h-12 w-full py-2 px-3 appearance-none border border-coolGray-200 rounded-lg shadow-md text-coolGray-300 leading-6 placeholder-coolGray-300 focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-green-500 focus:ring-opacity-50" type="file" placeholder="Dosya Seçilmedi"></div>
      </div>
     
      <button type="submit" onclick="return VerifyUploadSizeIsOK()" class="block py-5 px-10 mx-auto w-full md:max-w-max text-xl leading-6 text-white font-medium tracking-tighter font-heading text-center bg-blue-500 hover:bg-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-xl">Send</button>
    </form>
        ...
        @foreach ($files as $rs)
          <tr class="text-xs bg-gray-50">
            ...
            <td>
              ...
              <a class="inline-block" href="{{route('admin.admin_documents_delete',['id' =>$rs->id,'applications_id'=>$data->id])}}" onclick="return confirm('Delete ! Are you sure ?')">
