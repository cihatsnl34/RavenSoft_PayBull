...
<div class="max-w-4xl p-6 sm:p-10 mx-auto bg-coolGray-50 rounded-lg">
  <h1>Şirket ismi : {{$data->companyName}}</h1><br><br>
    <form action="{{ route('documents_add',['applications_id'=>$data->id]) }}" method="POST" enctype="multipart/form-data">
      ...
      <button  type="submit" class="block py-5 px-10 mx-auto w-full md:max-w-max text-xl leading-6 text-white font-medium tracking-tighter font-heading text-center bg-blue-500 hover:bg-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-xl">Evrak Yükle</button><br><br>
      <a href="{{route('documents_approve',['applications_id'=>$data->id])}}" class="block py-5 px-10 mx-auto w-full md:max-w-max text-xl leading-6 text-white font-medium tracking-tighter font-heading text-center bg-blue-500 hover:bg-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-xl">İşlemi Tamamla</a>
    </form>
  </div>
